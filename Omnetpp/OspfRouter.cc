...
void Router::rebuildRoutingTable()
{
    ...
    for (i = 0; i < routeCount; i++) {
        //EV_INFO<<"Route table pos "<<i<< " dest "<<routingTable[i]->getDestination().str()<<" gw "<<routingTable[i]->getGateway().str()<<endl;
        ...
        if (routingTable[i]->getDestinationType() == RoutingTableEntry::NETWORK_DESTINATION) {
            ...
            for(int j = 0; j < entry->getNextHopCount(); j++) {
                RoutingTableEntry * routeEntry = new RoutingTableEntry(*entry);
                routeEntry->setGateway(entry->getNextHop(j).hopAddress);
                routeEntry->setInterface(ift->getInterfaceById(entry->getNextHop(j).ifIndex));
                rt->addRoute(routeEntry);
            }
            //EV_INFO<<"Dodano wpis do tablicy ip "<<endl;
