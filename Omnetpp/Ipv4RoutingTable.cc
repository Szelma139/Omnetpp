...
Ipv4Route *Ipv4RoutingTable::findBestMatchingRoute(const Ipv4Address& dest) const
{
    ...
    */
    ...
    //EV_INFO<<"Router id ---- "<<routerId.str()<<endl;
    Ipv4Route *bestRoute = nullptr;
    std::vector<Ipv4Route *> listBestRoute;
    for (auto e : routes) {
        if (e->isValid()) {
            //EV_INFO << "Route dest: "<<e->getDestination().str()<<" brama "<<e->getGateway().str()<<endl;
            if (Ipv4Address::maskedAddrAreEqual(dest, e->getDestination(), e->getNetmask())) {    // match
                bestRoute = const_cast<Ipv4Route *>(e);
                listBestRoute.push_back(bestRoute);
                //break;
            }
        }
    }
    ...
    for(int i = 0; i < listBestRoute.size(); i++) {
        if(mapRouteUsage.count(listBestRoute[i]) != 0) {
            uint value = mapRouteUsage[listBestRoute[i]];
            if(value < minValue) {
                findIndexMin = i;
                minValue = value;
            }
        }else {
            mapRouteUsage[listBestRoute[i]] = 0;
            findIndexMin = i;
            minValue = 0;
        }
    }
    if(findIndexMin != -1){
        bestRoute = listBestRoute[findIndexMin];
        mapRouteUsage[listBestRoute[findIndexMin]]++;

    }
    //routingCache[dest] = bestRoute;
    std::string text = bestRoute !=nullptr ? bestRoute->str() : " null";
    EV_INFO<<"Dest "<<dest.str()<<" wybrana brama "<<text<<endl;
    return bestRoute;
}
...
{
    //EV_INFO<<" add route "<<entry->getDestination().str() <<" gw "<<entry->getGateway().str()<<endl;
    ...
    //EV_INFO<<"Dodano trase "<<entry->getDestination().str() <<" gw "<<entry->getGateway().str()<< " router "<<endl;
